...
gulp.task('templates', ['templates:clean'], function(){
  return gulp.src([Config.paths.source.components + '/**/*.html'])
    .pipe(ngTemplates('templates.js', {module: 'jsChallenge', basePath: ''}))
...
gulp.task('scripts:prod', ['templates', 'scripts:prod:clean'], function(){
  return gulp.src(appScripts)
    .pipe(concat(pkg.name+'.js', {
      separator: '\n\n',
      process: function(src) {
...
gulp.task('watch', function(){
  ...
  gulp.watch([Config.paths.source.root + '/**/*.js', Config.paths.source.components + '/**/*.html'], ['scripts:dev']);
